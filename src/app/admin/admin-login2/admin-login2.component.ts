...
import { AdminLogin2Service } from "./admin-login2-service";
...
@Component({
    ...
    templateUrl:'./admin-login2.html',
    styleUrls:['./admin-login2.css'],
    encapsulation:ViewEncapsulation.None
})
export class AdminLogin2 implements OnInit , AfterViewInit{
    ...
    public constructor(
        ...
        private classLoginData:AdminLoginService
    ...
    ngOnInit(){
        this.sendLocalStorageResultToServer();
    ...
    private loadLinkSliderData(link:string):void{
      ...
      this.adminLogin2.getClassDataByLink(data).subscribe(
          (res:HttpResponse<ResponseModel>)=>{
            let resonseModel:ResponseModel = res.body;
            if(resonseModel.result && resonseModel.data){
                this.loginPageData = <SliderData>resonseModel.data;
    ...
    public StudentLogin_SuccessHandler(response:HttpResponse<ResponseModel>):void{
        ...
        }else if(responseModel.result == false && responseModel.error_type == NOT_MATCH ){
          this.result.message = responseModel.message;
          this.result.color = "red";
          this.dialog.showAlert(responseModel.message ,small_dialog);

        }else if(responseModel.result == false && responseModel.error_type == EXCEPTION){
          this.isLoaded =  false;
          this.dialog.showAlert(responseModel.message ,small_dialog);
        }else if(responseModel.result == false && responseModel.error_type == RE_LOGIN){
    ...
    public instituteLogin_SuccessHandler(response:HttpResponse<ResponseModel>):void{
        this.isLoaded = false;
        let responseModel : ResponseModel = <ResponseModel>response.body;
        if( responseModel && responseModel.result){
            ...
            this.classLoginData.adminLoginData = classDetails;
    ...
    private instituteLogin():void{
        if(this.displayError() && this.checkWhoIsUser()==LoginType.INSTITUTE){
            ...
            this.adminService.login(data).subscribe({next:this.instituteLogin_SuccessHandler.bind(this),error:this.errorResponse_Handler.bind(this)});
        }else{
            this.dialog.showAlert("ERROR : Invalid Username or Password",{height:300,width:500});
        }
    }
            ...
            try{
              ...
              if(encryptedResult != null && encryptedResult != undefined && encryptedResult.trim().length > 0){
                ...
                if(jsonString != null && jsonString != undefined && jsonString.trim().length > 0){
                  ...
                  this.isLoaded = true;
                  this.loginService.submitLocalTestResult(result).subscribe((res:HttpResponse<ResponseModel>)=>{
    ...
    private setData(res:ResponseModel):void{
    ...
    private checkWhoIsUser():LoginType{
        ...
        let userName = (!this.userNameControl.value)?''.trim():this.userNameControl.value.trim();
        if(institutePattern.test(userName)){
            return LoginType.INSTITUTE;
        }else if(studentPattern.test(userName)){
